import { blend, BLEND_MODES } from "../src/blend.js";
import { Noise } from "../src/noise/noise.js";
import { CurveEdit } from "./curve.js";

export function main() {
    const layerTypes = {
        "simplex noise 2": Noise
    };
    ...
    function createLayerElement(id) {
        ...
        Object.keys(BLEND_MODES).forEach(mode => {
            const option = document.createElement('option');
            option.value = mode;
    ...
    function getLayerConfigs() {
        return Array.from(layersContainer.children).map(layer => {
            ...
            return { generator, type, blend, params, opacity, curve: curve.get256() };
        }).filter(Boolean);
    }
    ...
    function render() {
        ...
        for (const layer of config) {
            ...
            const blendMode = BLEND_MODES[layer.blend];
            for (let y = 0; y < 800; y++)
                for (let x = 0; x < 800; x++) {
                    const idx = y * 800 + x;
                    data[idx] = blend(data[idx], generator.gen(x, y), blendMode, layer.opacity);
                }
